...
- **CLI:** `parse`, `leaks`, `analyze`, `ci-check`, `flamegraph`, `diff`, `map`, `query`, `gc-path`, `explain`, `chat`, `fix`, `snapshot`, `serve`, and `config` all call the shared core. Reports emit via stdout or `--output-file`, except `ci-check`, which uses `--output` for its policy-gate artifacts, and `flamegraph`, which requires `-o/--output` because the artifact is SVG, folded-stack text, or JSON. `parse`, `analyze`, `ci-check`, and `flamegraph` all use `--mode auto|deep|overview` where appropriate; `auto` resolves by file size with a 4 GiB default threshold that can be overridden via `MNEMOSYNE_OVERVIEW_AUTO_THRESHOLD`, while `overview` switches to graph-free streaming triage. `ci-check` adds `--policy`, `--format text|json|junit|github-actions`, and `--fail-on info|warning|error|critical`; `flamegraph` adds `--root dominator|class-hierarchy|gc-root-path`, `--format svg|folded-stack|json`, `--min-fraction`, `--title`, and `--max-frames`, and exits `5` when the user requests or auto-resolves to overview mode; the `analyze` command accepts `--group-by class|package|classloader|superclass`, `--threads`, `--strings`, `--collections`, `--duplicate-arrays`, `--top-instances`, `--top-n`, and `--min-capacity`; `chat` now starts a CLI-only leak-focused conversation session that analyzes once, shows the top 3 leak candidates, and supports `/focus <leak-id>`, `/list`, `/help`, and `/exit`; `leaks` now prints an explicit zero-result confirmation; and `diff` now prints class-level retained deltas when graph-backed diffing succeeds.
- **Query engine:** `core::query` carries the M7-4 targeted parity slice plus the M15 bounded expansion (Slices 15.C–15.E): built-in pseudo-attributes `@retainedSize`, `@toString`, and `@gcRootPath`; `LIKE` / `CONTAINS` / regex `=~`; `OBJECTS` single-hop projection; `IS NULL` / `IS NOT NULL`; single-quoted string literals; traversal functions `outbounds(...)`/`inbounds(...)`/`dominators(...)`; one-level subqueries (`FROM OBJECTS (SELECT ...)`); and `UNION` with object-id deduplication. Named deferrals: `eval(...)`, multi-class `FROM`, arbitrary-depth nesting, multi-hop `OBJECTS`. `core::query::synth` owns synthetic string rendering so the query layer can decode real `java.lang.String` contents when field data is available and fall back honestly elsewhere.
...
- **MAT backend parity completion (M15):** `core::analysis::array_analysis` (`analyze_duplicate_arrays()`, `DuplicateArrayGroup`, optional `ArrayReport` on `AnalyzeResponse`, gated by `enable_duplicate_arrays`) mirrors string duplicate detection for primitive arrays. `HistogramGroupBy::Superclass` groups histograms by immediate superclass via bounded `super_class_id` walk. OQL gains traversal functions, regex `=~`, one-level subqueries, and `UNION` in `core::query` (see Query engine bullet). `core::plugin` ships Phase-2 static registration (`AnalyzerPlugin`, `ReportFormatterPlugin`, `PluginRegistry`, `analyze_heap_with_plugins`, `render_report_with_plugins`, `OutputFormat::Custom(name)`); Phase 3 dynamic loading remains deferred. See [docs/design/milestone-15-mat-backend-parity.md](docs/design/milestone-15-mat-backend-parity.md).
...
- **Desktop app (M16):** `tauri/` wraps the shared `ui/` build, injects both pre-M14 host bridges, and exposes native commands for `load_heap`, `unload_heap`, `query_heap`, `get_references`, `get_referrers`, `explain_leak`, `find_gc_path`, `map_to_code`, and `propose_fix`. Tagged-release CI (`build-desktop` job **configured** in `release.yml`) targets Tauri installers (`.msi`/`.exe`, `.dmg`, `.deb`/`.AppImage`/`.rpm`) on post-M16 tags — **unsigned by default** until release signing secrets are configured; Windows launch-tested locally, macOS/Linux not launch-tested; no in-app auto-updater in v1. M14's comparison/workflow bridges and `inspectObject`/`findAllGcPaths` still have no Tauri native-command wiring.
...
**Desktop distribution (M16, shipped with caveats)**: `tauri/` wraps the shared `ui/` React frontend and injects the two pre-M14 host bridges through native commands. M16 adds tagged-release CI packaging (`build-desktop` job **configured** in `release.yml`) — targets post-M16 GitHub Releases, **unsigned by default** until signing secrets are configured, Windows launch-tested locally, macOS/Linux not launch-tested, no in-app auto-updater in v1. M14's two further bridges (comparison, workflow) and `inspectObject`/`findAllGcPaths` on the original two still have no Tauri native-command equivalent — optional follow-up wiring.
...
**Custom Analysis Plugins**: M15 Slice 15.F shipped Phase-2 static registration (`core::plugin`: `AnalyzerPlugin`, `ReportFormatterPlugin`, `PluginRegistry`) — any Rust project depending on `mnemosyne-core` can register custom analyzers/formatters at compile time via `analyze_heap_with_plugins` / `render_report_with_plugins`. Phase 3 dynamic `cdylib` loading remains deferred per `docs/design/m6-plugin-extension-system.md`. Alternatively, extending the AI prompts: allow users to add their own prompt templates for domain-specific analysis.
