...
fn build_medium_fixture_pair() -> SyntheticFixturePair {
...
fn run_object_diff(
    ...
    after_path: &str,
    strategy: IdentityStrategy,
) -> mnemosyne_core::HeapDiff {
    let request = object_diff_request(before_path, after_path, strategy);
    match runtime
        .block_on(run_diff(request))
        .expect("object diff benchmark should succeed")
    {
...
fn diff_object_benches(c: &mut Criterion) {
    let pair = build_medium_fixture_pair();
    let materialized = materialize_fixture_pair(&pair);
    let runtime = Runtime::new().expect("tokio runtime for diff benches");
    let throughput = u64::try_from(pair.before.len() + pair.after.len()).expect("pair size");

    let mut group = c.benchmark_group("diff_object");
    group.throughput(Throughput::Bytes(throughput));

    group.bench_function("class_diff_baseline", |bench| {
        bench.iter(|| {
            let diff = runtime
                .block_on(diff_heaps(
                    black_box(materialized.before_path.as_str()),
                    black_box(materialized.after_path.as_str()),
                ))
                .expect("class diff benchmark should succeed");
            black_box(diff)
        })
    });

    group.bench_function("object_diff_class_retained", |bench| {
        bench.iter(|| {
            let diff = run_object_diff(
                &runtime,
                black_box(materialized.before_path.as_str()),
                black_box(materialized.after_path.as_str()),
    ...
    group.bench_function("object_diff_class_dominator", |bench| {
        bench.iter(|| {
            let diff = run_object_diff(
                &runtime,
                black_box(materialized.before_path.as_str()),
...
criterion_group!(benches, diff_object_benches);
