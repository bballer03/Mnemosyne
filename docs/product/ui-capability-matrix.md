...
| Desktop asset name/integrity verify | `verify_desktop_assets.py` + `generate_sha256sums.py` + `normalize_desktop_assets.py` | **partial** | M21.C/D normalize Tauri→frozen AppImage/DMG/app-zip names in CI; SHA256SUMS + warn-mode verify; **not** strict fail gate or launch proof |
| macOS app zip / Linux AppImage names | `normalize_desktop_assets.py` + `release.yml` | **partial** | Frozen names documented + CI rename/zip wired; native launch / Info.plist / AppImage arch probes **not proven** |
