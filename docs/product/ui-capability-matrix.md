...
| Desktop asset name/integrity verify | `verify_*` + `generate_sha256sums` + `inspect_desktop_archives` + `normalize_*` | **partial** | Unsigned desktop-scope SHA256SUMS + CI provenance; zip secret/build-path inspect fail-closed; warn-mode name verify; **not** strict name gate or launch proof |
| macOS app zip / Linux AppImage names | `normalize_desktop_assets.py` + `release.yml` | **partial** | Frozen names + ditto-only macOS zip on runners; AppImage/DMG rename; native launch / Info.plist / arch probes **not proven** |
