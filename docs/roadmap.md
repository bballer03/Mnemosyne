...
| AI / LLM insights | ⚠️ Partial | `core::analysis::generate_ai_insights()` now supports `rules`, `stub`, and `provider` modes with ordered task definitions from `AiConfig`, preserving CLI/MCP/report contracts. OpenAI-compatible, local, and Anthropic provider execution now have targeted verification coverage. Step `14(d)` now covers prompt redaction, hashed audit logging, and a minimal prompt-budget guard, and Step `14(e)` shipped a CLI-first chat slice with bounded in-process history and top-3 shortlist context plus an AI-backed one-file / one-snippet fix-generation slice; richer MCP/session semantics and broader AI hardening still remain. |
...
| 16 | LLM integration (real API calls) | P1 | High | L | M3 analysis context | M5 | ⚠️ Partial — provider mode, YAML prompt templates, provider-mode prompt redaction, provider audit logging, a minimal prompt-budget guard, MCP tool/error hardening, CLI-first chat, and the first AI-backed fix-generation slice landed; MCP/session follow-through and broader transport hardening still remain |
