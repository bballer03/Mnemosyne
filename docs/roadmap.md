...
| OQL — full operator set | ✅ | 🟡 | M7-4 + M15 + M22 closed the bounded high-value slice; named MAT gaps remain | Low | Shipped (M7-4/M15/M22): see [evidence/m22-oql-compatibility.md](evidence/m22-oql-compatibility.md). Still deferred (named, not silent): `eval(...)`, arbitrary-depth subquery nesting. Multi-class `FROM` and 1–3 hop `OBJECTS` shipped in M22 (not silent deferrals). |
...
- **MAT ≥ Mnemosyne (low priority / named deferrals):** remaining OQL tail (`eval(...)`, arbitrary-depth subquery nesting); Phase 3 dynamic plugin loading (Phase 2 static registry shipped in M15). Multi-class `FROM` and 1–3 hop `OBJECTS` shipped in M22 (see [evidence/m22-oql-compatibility.md](evidence/m22-oql-compatibility.md)).
...
- **Not shipped (explicit named deferrals, per design doc §4):** `eval(...)` scriptlets, arbitrary-depth subquery nesting, Phase-3 dynamic `cdylib` plugin loading, UI surfacing of any M15 backend item (M14 backend-before-UI pattern). Multi-class `FROM` and multi-hop `OBJECTS` were listed here at M15 ship time — **since resolved:** M22.B–C (corpus evidence [m22-oql-compatibility.md](evidence/m22-oql-compatibility.md)).
...
| **UI → MAT maturity program (M24–M32)** | [superpowers/specs/2026-09-15-ui-mat-maturity-roadmap-design.md](superpowers/specs/2026-09-15-ui-mat-maturity-roadmap-design.md) | 🟡 M31 Wave 1 active after M25–M30 closeout; [dependency inventory](product/dependency-currency-inventory.md) |
| **M31.B/C MAT golden + perspectives** | [program design](superpowers/specs/2026-09-15-eclipse-mat-equivalent-program-design.md), [31.B evidence](evidence/m31b-mat-golden.md), [31.C evidence](evidence/m31c-perspectives.md) | 🟡 Three `mnemosyne-golden` cases and four persisted fixed perspectives landed; `mat-referenced` = 0 and packaged GUI remain **NOT PROVEN** |
