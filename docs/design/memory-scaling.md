...
> Status: Active - Step 11 complete
...
- Dense synthetic multi-tier validation now measured the lean path at 2.87x-2.90x and the investigation path at 3.89x-3.92x across ~500 MB, ~1 GB, and ~2 GB tiers.
...
- **Current decision:** keep the in-memory `ObjectGraph` architecture for the current milestone; Step 11 has cleared it through the validated ~2 GB tier.
- **Rationale:** Step 11 confirmed that unconditional field retention regressed the 156 MB fixture to 4.78x RSS:dump, and the implemented `ParseOptions` remediation reduced default `analyze`/`leaks` runs to 4.23x there. The dense synthetic multi-tier validation then showed materially better large-tier behavior: default `analyze`/`leaks` stayed at 2.87x-2.90x and investigation-heavy runs stayed at 3.89x-3.92x across ~500 MB, ~1 GB, and ~2 GB tiers.
...
- **Follow-up work:** keep streaming overview mode and disk-backed alternatives on the roadmap as future scale levers, but they are no longer blockers for the current M3 gate.
...
3. **Synthetic-vs-real-world caveat remains:** dense synthetic heaps now validate the architecture through ~2 GB, but additional real-world large-dump validation would still improve confidence.
...
Still pending:
1. Re-measuring `gc-path` explicitly on a larger dense tier if a dedicated memory profile is needed
...
Step 11 is a validation and decision gate. The re-baseline, remediation, 156 MB post-remediation validation, dense synthetic generator redesign, and 500 MB / 1 GB / 2 GB validation tiers are now complete.
...
- **Done:** multi-tier validation at ~500 MB / ~1 GB / ~2 GB using dense synthetic heaps
...
### Multi-Tier Validation Result (2026-04-12)

Dense synthetic heaps were generated with the redesigned `SyntheticHeapApp.java` so dump growth materially increased retained object count and reference density rather than just a few large payload arrays.

| Tier | Actual Dump Size | Default `analyze` / `leaks` | Investigation `analyze --threads --strings --collections` | Outcome |
|------|------------------|-----------------------------|------------------------------------------------------------|---------|
| T1 | 494.39 MiB | 2.90x | 3.92x | PASS |
| T2 | 981.68 MiB | 2.87x | 3.89x | PASS |
| T3 | 1.88 GiB | 2.89x | 3.92x | PASS |

### Final Step 11 Decision

- **Decision:** PASS - No action
- **Why:** The lean default path stayed well below the 4.0x threshold at T1 and T2 and remained stable at T3. The investigation-heavy path also stayed below 4.0x at all three validated tiers.
- **Architectural conclusion:** the current in-memory `ObjectGraph` approach is acceptable for the current roadmap scope. Streaming overview mode and disk-backed alternatives remain future improvements rather than active blockers.
