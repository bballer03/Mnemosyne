...
> **Status:** ✅ **Shipped** — Slices 11.A–11.D (implementation) and 11.E (this doc-sync pass) all complete. See §14 for the closeout note.
...
> **Predecessors:** M8 (Reachability & References) ✅ shipped, M10 (Object-Level Diff) ✅ mostly shipped, M9 (Snapshot Persistence) ✅ shipped by the time Slice 11.D landed (Slices 9.A–9.D all merged ahead of it — see §14).
> **Last updated:** 2026-08-18 (Slice 11.E doc-sync)
...
- Architecture: [ARCHITECTURE.md](../../ARCHITECTURE.md) — updated in Slice 11.E (this pass).
...
## 14. Slice 11.E closeout (documentation sync)

Slices 11.A–11.D landed as: `509fbe1` (11.A, `core::workflow` scaffolding — `WorkflowState`/`WorkflowStore`/`StepRecord`/`WorkflowDescription`/`describe()`/`start()`/`advance()` — plus the first fully-working kind, `TriageMemoryLeak`), `7f9ad28` (11.B, `TuneGc` and `TraverseObjectGraph`, including `TraverseObjectGraph`'s `choose_direction` branch-point validation), `0c9fcf0` (11.C, `CompareSnapshots`, the fourth and final `WorkflowKind`, composing `SnapshotStore` + `run_diff`/`DiffMode::Object`), and `85862a6` (11.D, all five MCP tools registered in `core::mcp::server` plus `docs/mcp-workflows.md`'s captured transcripts). Slice 11.E (this pass) updated: `docs/roadmap.md` (M11 marked shipped in §5 with delivered-vs-speculative scope called out, the scorecard §7, and the design-doc index §9 — plus correcting two stale `(M11)` priority-tag mislabels in §2's parity matrix for items — duplicate-array detection, group-by-superclass — that were never actually part of this milestone's real scope), `STATUS.md` (new M11 snapshot bullet plus two capability-checklist rows), `CHANGELOG.md` (`[Unreleased]` entry appended below the existing M8/M9/M10/M13 entries), `README.md` (MCP workflow suite bullet under IDE Integration, all five new tools added to the method list and the descriptive-paragraph block, and the stale "18 methods" architecture-summary count corrected to the actual current total of 24 — a gap that predated this milestone and was not specific to M11's own additions, but was caught while touching this same block, the same way M9's own doc-sync caught a pre-existing `diff_heaps` gap), `ARCHITECTURE.md` (new "Shipped today" bullet for `core::workflow`, plus a `workflow/` branch in the project-structure tree sibling to `snapshot/`), and `docs/user-guide.md` (new §7.1 "MCP workflow suite" subsection describing all five tools and the four workflow kinds, linking out to `docs/mcp-workflows.md` for the worked transcripts rather than duplicating them, plus the five tools added to the MCP method list).

**Scope drift found during doc-sync (this design doc's own speculative text vs. what actually shipped):**

- **No `[workflow].directory` config-key override shipped.** §4 point 3 speculated a TOML config-key override "mirroring `[ai.sessions].directory`" alongside an env var. What shipped (`core::mcp::server::default_workflow_dir()`) is env-var-only — `MNEMOSYNE_WORKFLOW_DIR` — mirroring `MNEMOSYNE_SNAPSHOT_DIR`'s own precedent, which M9's own Slice 9.E doc-sync already recorded as the actually-established pattern (no config key ever landed for snapshots either). `default_workflow_dir()`'s own doc comment names this explicitly as a deliberate deviation from this design doc's speculative text, not an oversight.
- **§9 R3's "reconciliation pass only if real drift is observed" resolved to "no reconciliation needed."** `CompareSnapshots` was scoped against `core::snapshot` directly rather than a not-yet-existing CLI/MCP-wired surface (§4's explicit non-blocking-dependency note). By the time Slice 11.D actually landed, M9 Slices 9.C/9.D had already shipped, so `start_workflow`'s `before_snapshot_key`/`after_snapshot_key` params were added straightforwardly using the now-finished `SnapshotStore` semantics with no observed drift to reconcile — the risk register's conditional resolved in the "no action needed" branch, not the "reconciliation pass" branch.
- **`WorkflowStore::list()` was not shipped, exactly as anticipated.** §6 explicitly scoped this as optional ("add `list()` only if Slice 11.A's own implementer finds it trivially cheap to include, not a hard requirement"). It wasn't added — `get_workflow` on a known id proved sufficient, matching the roadmap's own MCP surface (§7), which never specified a `list_workflows` tool. Not a gap; a correctly-resolved design option.
- Everything else — the four `WorkflowKind` step sequences exactly as specified in §4/§8, the `WorkflowState`/`StepRecord`/`WorkflowDescription` data model in §6, the five-tool MCP surface and its `error_details` codes in §7, the §6.1 contract-test discipline (verified for all four kinds), and the §3.3 "zero new heap-analysis logic" design principle itself (verified as a regression gate: every step's data traces back to an existing, already-tested primitive) — shipped exactly as this design doc specified.

Full-workspace verification at closeout: `cargo check --workspace --all-targets` clean, `cargo test --workspace` 714 passed / 0 failed (28 test binaries, 0 doctests), `cargo clippy --workspace --all-targets -- -D warnings` clean, `cargo fmt --all -- --check` clean. Note: a flaky-test race in `MNEMOSYNE_SNAPSHOT_DIR`-mutating tests was fixed on `main` (commit `df51e5c`, "serialize `MNEMOSYNE_SNAPSHOT_DIR` mutation crate-wide") immediately ahead of this slice; this closeout's verification run landed clean on the first pass with that fix already in place, so no flake was observed or needed re-running past.
